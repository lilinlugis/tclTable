...
**多行表头的实现有些复杂，为不影响tcl-simple-table指令，创建了两个新的指令tclTableCustomHeader和tclTableMultiColumns来实现，这两个指令都可实现多行表头，前者允许用户自定义模板，使用起来更灵活，后者则仍然通过列定义来实现，只不过columns变成了二维数组。**

### compositeData参数
compositeData:true可以实现嵌套的数据，合并的字段包含了被合并字段的数据，注意data的定义。

## tclTableCustomHeader
```html
<h2>通过模板字符串自定义表头</h2>
<!--方案一，模板写在scope变量上-->
<div class="table-overflow" simple-table-custom-header="dataTable0" tcl-table-pagination header-template="customHeader"></div>
<h2>通过模板文件自定义表头</h2>
<!--模板写在独立文件里-->
<div class="table-overflow" simple-table-custom-header="dataTable1" tcl-table-pagination header="template/test.html"></div>
```
...
```
## tclTableMultiColumns
```html
<h2>通过columns自定义表头(复合数据)</h2>
<div class="table-overflow" simple-table-multi-columns="dataTable2"  tcl-table-pagination></div>
<h2>通过columns自定义表头(简单数据)</h2>
<div class="table-overflow" simple-table-multi-columns="dataTable3"  tcl-table-pagination></div>
```
```js
 $scope.dataTable2={
    compositeData:true,
...
在定义二维的columns时，一定要准确定义rowspan和colspan才能达到想要的效果，注意colspan的值是其包含的物理列的数量，而不是它下一行的列数量，比如上面的列2017里面包含的物理列为26，而不是被它合并的6个单元格，因为它合并的6个单元格下面还有具体的列。所谓的物理列是指具体被渲染的列，这些列是没有再合并其他列的，也就是colspan=1的列。

compositeData的值根据数据的接口定义来设置，如果后台给出的数据没有嵌套，可不做设置。
